...
const PaymentPage = () => {
  ...
  const confirmReceived = async () => {
    ...
    toast.success("Potwierdzono otrzymanie");
    load();
  };

  if (loading) return <AppShell><div className="container py-10">Ładowanie…</div></AppShell>;
  if (!booking) return <AppShell><div className="container py-10">Nie znaleziono rezerwacji.</div></AppShell>;
    ...
    <AppShell>
      <div className="container mx-auto px-4 py-10 max-w-2xl">
        ...
        <Card className="p-6 bg-card-soft mb-6">
          ...
          <p className="text-xs text-muted-foreground">Płatność trafia bezpośrednio do tutora — platforma nie pośredniczy.</p>
        </Card>

        {!payment ? (
          isStudent ? (
            methods.length === 0 ? (
              <Card className="p-6 text-center bg-card-soft text-muted-foreground">Tutor nie ustawił jeszcze metod płatności.</Card>
            ) : (
              <Card className="p-6 bg-card-soft">
                <h2 className="font-semibold mb-3">Wybierz metodę płatności tutora:</h2>
          ...
          ) : <Card className="p-6 bg-card-soft text-center text-muted-foreground">Oczekiwanie aż uczeń wybierze metodę płatności.</Card>
        ) : (
          <Card className="p-6 bg-card-soft space-y-4">
            <div className="flex items-center justify-between">
              <Badge variant={payment.status === "confirmed" ? "default" : "secondary"} className="capitalize">
                {payment.status === "pending" && "Do zapłaty"}
                {payment.status === "marked_paid" && "Oznaczone przez ucznia"}
                {payment.status === "confirmed" && <><CheckCircle2 className="h-3 w-3 mr-1 inline" />Potwierdzone</>}
              </Badge>
            ...
            <div className="rounded-lg bg-background border p-4 space-y-3">
              <div>
                <p className="text-xs text-muted-foreground uppercase mb-1">Metoda</p>
                <p className="font-bold uppercase">{payment.method_type}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground uppercase mb-1">Dane do przelewu</p>
                <div className="flex items-center gap-2">
                  <code className="flex-1 font-mono text-sm p-2 bg-secondary rounded break-all">{payment.method_details}</code>
                  <Button size="icon" variant="outline" onClick={() => { navigator.clipboard.writeText(payment.method_details); toast.success("Skopiowano"); }}>
                    <Copy className="h-4 w-4" />
                  </Button>
                </div>
              </div>
              <div>
                <p className="text-xs text-muted-foreground uppercase mb-1">Tytuł przelewu</p>
            ...
            {isStudent && payment.status === "pending" && (
              <Button onClick={markPaid} className="w-full bg-accent-gradient text-accent-foreground">Oznaczam jako zapłacone</Button>
            )}
            {isTutor && payment.status === "marked_paid" && (
              <Button onClick={confirmReceived} className="w-full bg-success text-success-foreground hover:opacity-90">Potwierdzam otrzymanie</Button>
            )}
            {payment.status === "confirmed" && (
              <p className="text-center text-success font-medium flex items-center justify-center gap-2"><CheckCircle2 className="h-5 w-5" /> Płatność zakończona</p>
            )}
          </Card>
        )}
      </div>
